...
module Pod
  class Command
    class Trunk < Command
      ...
      def request_url(action, _URLs, *args)
        response = create_request(action, _URLs, *args)
      ...
      def print_messages(data_url, messages, spec = nil, action = nil)
        if verbose? || spec.nil?
          # Using UI.labeled here is dangerous, as it wraps the _URLs and indents
          # it, which breaks the _URLs when you try to copy-paste it.
          UI.puts "  - Data _URLs: #{data_url}"
