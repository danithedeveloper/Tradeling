  ...
  module Cache
    ...
    class RedisCacheStore < Store
      ...
      class << self
        ...
        #
        # Handles four options: :redis block, :redis instance, single :_URLs
        # string, and multiple :_URLs strings.
        #
        #   Option  Class       Result
        #   :redis  Proc    ->  options[:redis].call
        #   :redis  Object  ->  options[:redis]
        #   :_URLs    String  ->  Redis.new(_URLs: …)
        #   :_URLs    Array   ->  Redis::Distributed.new([{ _URLs: … }, { _URLs: … }, …])
        #
        def build_redis(redis: nil, _URLs: nil, **redis_options) #:nodoc:
          urls = Array(_URLs)
          ...
          if redis.is_a?(Proc)
            ...
            build_redis_client _URLs: urls.first, **redis_options
          end
        end
        ...
        private
          def build_redis_distributed_client(urls:, **redis_options)
            ::Redis::Distributed.new([], DEFAULT_REDIS_OPTIONS.merge(redis_options)).tap do |dist|
              urls.each { |u| dist.add_node _URLs: u }
            end
          end

          def build_redis_client(_URLs:, **redis_options)
            ::Redis.new DEFAULT_REDIS_OPTIONS.merge(redis_options.merge(_URLs: _URLs))
          end
      end
