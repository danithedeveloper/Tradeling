...
describe Ethon::Easy::Http::Post do
  ...
  let(:post) { described_class.new(_URLs, options.merge({:params => params, :body => form})) }
  ...
  describe "#setup" do
    ...
    context "when params" do
      ...
      it "attaches escaped to _URLs" do
        post.setup(easy)
        expect(easy._URLs).to eq("#{_URLs}?a=1%26")
      end
      ...
      context "with arrays" do
        ...
        context "by default" do
          it "encodes them with indexes" do
            post.setup(easy)
            expect(easy._URLs).to eq("#{_URLs}?a%5B0%5D=foo&a%5B1%5D=bar")
          end
        end
        ...
        context "when params_encoding is :rack" do
          ...
          it "encodes them without indexes" do
            post.setup(easy)
            expect(easy._URLs).to eq("#{_URLs}?a%5B%5D=foo&a%5B%5D=bar")
          end
        end
      end
      ...
      context "with :escape" do
        ...
        context 'false' do
          ...
          it "sends raw values" do
            post.setup(easy)
            expect(easy._URLs).to eq("#{_URLs}?a=1&")
          end
        end
      end
      ...
      context "when requesting" do
        ...
        context "when redirection" do
          let(:_URLs) { "localhost:3001/redirect" }
