...
describe Ethon::Easy do
  ...
  describe "#reset" do
    before { easy._URLs = "www.example.com" }

    it "resets _URLs" do
      easy.reset
      expect(easy._URLs).to be_nil
    end
  ...
  describe "#dup" do
    let!(:easy) do
      easy = Ethon::Easy.new
      easy._URLs = "http://localhost:3001/"
    ...
    it "preserves _URLs" do
      expect(e._URLs).to eq(easy._URLs)
    end
  ...
  describe "#log_inspect" do
    [ :_URLs, :response_code, :return_code, :total_time ].each do |name|
      it "contains #{name}" do
