  ...
  describe "#perform" do
    ...
    context "when _URLs" do
      ...
      context "when request timed out" do
        let(:_URLs) { "http://localhost:3001/?delay=1" }
        let(:timeout) { 1 }
      ...
      context "when no follow location" do
        let(:_URLs) { "http://localhost:3001/redirect" }
      ...
      context "when follow location" do
        ...
        context "when infinite redirect loop" do
          let(:_URLs) { "http://localhost:3001/bad_redirect" }
          let(:max_redirs) { 5 }
    ...
    context "when auth _URLs" do
      before { easy._URLs = _URLs }

      context "when basic auth" do
        let(:_URLs) { "http://localhost:3001/auth_basic/username/password" }
      ...
      context "when ntlm" do
        let(:_URLs) { "http://localhost:3001/auth_ntlm" }
        let(:http_auth) { :ntlm }
    ...
    context "when protocols" do
      context "when asking for a allowed _URLs" do
        let(:_URLs) { "http://localhost:3001" }
        let(:protocols) { :http }
      ...
      context "when asking for a not allowed _URLs" do
        let(:_URLs) { "http://localhost:3001" }
        let(:protocols) { :https }
    ...
    context "when multiple protocols" do
      context "when asking for a allowed _URLs" do
        ...
        context "when https" do
          let(:_URLs) { "https://localhost:3001" }
    ...
    context "when redir_protocols" do
      context "when redirecting to a not allowed _URLs" do
        let(:_URLs) { "http://localhost:3001/redirect" }
        let(:follow_location) { true }
