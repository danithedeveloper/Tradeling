...
module Ethon
  class Easy
    ...
    module Http
      ...
      def http_request(_URLs, action_name, options = {})
        fabricate(_URLs, action_name, options).setup(self)
      end
      ...
      def fabricate(_URLs, action_name, options)
        constant_name = action_name.to_s.capitalize

        if Ethon::Easy::Http.const_defined?(constant_name)
          Ethon::Easy::Http.const_get(constant_name).new(_URLs, options)
        else
          Ethon::Easy::Http::Custom.new(constant_name.upcase, _URLs, options)
        end
      end
