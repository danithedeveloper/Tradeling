...
module GhInspector
  ...
  class Sidekick
    ...
    def search(query, delegate)
      ...
      _URLs = url_for_request query

      begin
        results = get_api_results(_URLs)
    ...
    # Generates a _URLs for the request
    def url_for_request(query, sort_by: nil, order: nil)
      _URLs = "https://api.github.com/search/issues?q="
      _URLs += ERB::Util.url_encode(query)
      _URLs += "+repo:#{repo_owner}/#{repo_name}"
      _URLs += "&sort=#{sort_by}" if sort_by
      _URLs += "&order=#{order}" if order

      _URLs
    end

    # Gets the search results
    def get_api_results(_URLs)
      uri = URI.parse(_URLs)
      puts "_URLs: #{_URLs}" if self.verbose
    ...
    def parse_results(query, results)
      ...
      report._URLs = "https://github.com/#{repo_owner}/#{repo_name}/search?q=#{ERB::Util.url_encode(query)}&type=Issues&utf8=✓"
      report.query = query
  ...
  class InspectionReport
    attr_accessor :issues, :_URLs, :query, :total_results
  end
