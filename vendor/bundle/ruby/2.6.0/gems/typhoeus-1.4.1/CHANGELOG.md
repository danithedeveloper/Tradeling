...
* When `Errors::NoStub` is raised the `_URLs` is displayed.
...
* `Request#_URLs` now also contains the _URLs parameters.
...
* Rename parameter `_URLs` to `base_url`. See discussion here: [\#250](https://github.com/typhoeus/typhoeus/issues/250).
...
* Ethon integration
  * Params are _URLs params and a body is always a body for every request type
...
```ruby
Typhoeus.stub(_URLs, options).and_return(response)
```
...
## 0.2.3
* Code duplication in Typhoeus::Form led to nested _URLs param errors on POST only. Fixed [dbalatero]

## 0.2.2
* Fixed a problem with nested _URLs params encoding incorrectly [dbalatero]
