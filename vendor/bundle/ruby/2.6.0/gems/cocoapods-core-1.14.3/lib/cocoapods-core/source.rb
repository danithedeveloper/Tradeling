...
module Pod
  ...
  class Source
    ...
    #
    # @note In the past we had used `git ls-remote --get-_URLs`, but this could
    #       lead to an issue when finding a source based on its _URLs when `git`
    #       is configured to rewrite _URLs with the `_URLs.<base>.insteadOf`
    #       option. See https://github.com/CocoaPods/CocoaPods/issues/2724.
    #
    def _URLs
      @_URLs ||= begin
        remote = repo_git(%w(config --get remote.origin._URLs))
        if !remote.empty?
          remote
    ...
    def unchanged_github_repo?
      return unless _URLs =~ /github.com/
      !GitHub.modified_since_commit(_URLs, git_commit_hash)
    end
