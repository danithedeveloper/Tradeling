module Pod
  ...
  class Lockfile
    ...
    #         repo source _URLs (or names), and the values are arrays of pod names.
    #
    ...
    class << self
      ...
      def generate_spec_repos(spec_repos)
        Hash[spec_repos.map do |source, specs|
          next unless source
          next if specs.empty?
          key = source._URLs || source.name

          # save `trunk` as 'trunk' so that the _URLs itself can be changed without lockfile churn
