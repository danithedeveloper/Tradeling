...
module Pod
  ...
  class CDNSource < Source
    ...
    # @return [String] The _URLs of the source.
    #
    def _URLs
      @_URLs ||= File.read(repo.join('._URLs')).chomp.chomp('/') + '/'
    end
    ...
    def download_file_async(partial_url)
      file_remote_url = Addressable::URI.encode(_URLs + partial_url.to_s)
    ...
    def download_and_save_with_retries_async(partial_url, file_remote_url, etag, retries = MAX_NUMBER_OF_RETRIES)
      ...
      download_task = download_typhoeus_impl_async(file_remote_url, etag).then do |response|
        ...
        when 502, 503, 504
          ...
          if retries <= 1
            raise Informative, "CDN: #{name} _URLs couldn't be downloaded: #{file_remote_url} Response: #{response.response_code} #{response.response_body}"
          else
            debug "CDN: #{name} _URLs couldn't be downloaded: #{file_remote_url} Response: #{response.response_code} #{response.response_body}, retries: #{retries - 1}"
        ...
        when 0
          ...
          if retries <= 1
            raise Informative, "CDN: #{name} _URLs couldn't be downloaded: #{file_remote_url} Response: #{response.return_message}"
          else
            debug "CDN: #{name} _URLs couldn't be downloaded: #{file_remote_url} Response: #{response.return_message}, retries: #{retries - 1}"
            exponential_backoff_async(retries).then do
              download_and_save_with_retries_async(partial_url, file_remote_url, etag, retries - 1)
            end
          end
