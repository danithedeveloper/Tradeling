...
module Pod
  class Source
    class Manager
      ...
      #
      def source_with_url(_URLs)
        _URLs = canonic_url(_URLs)
        _URLs = 'https://github.com/cocoapods/specs' if _URLs =~ %r{github.com[:/]+cocoapods/specs}
        all.find do |source|
          source._URLs && canonic_url(source._URLs) == _URLs
        end
      end

      def canonic_url(_URLs)
        _URLs.downcase.gsub(/\.git$/, '').gsub(%r{\/$}, '')
      end

      # Returns a suitable repository name for `_URLs`.
      #
      # @example A GitHub.com _URLs
      #
      ...
      #            # "artsy-1"
      #
      # @example A non-Github.com _URLs
      #
      #          name_for_url('https://sourceforge.org/Artsy/Specs.git')
      #            # sourceforge-artsy
      #
      # @example A file _URLs
      #
      ...
      #         The _URLs of the source.
      #
      # @return [String] A suitable repository name for `_URLs`.
      #
      def name_for_url(_URLs)
        ...
        valid_url = lambda do |_URLs|
          _URLs =~ URI.regexp && (URI(_URLs) rescue false)
        end

        valid_scp_url = lambda do |_URLs|
          valid_url['scp://' + _URLs]
        end

        _URLs = _URLs.to_s.downcase
        ...
        when valid_url
          # HTTPS _URLs or something similar
          _URLs = valid_url[_URLs]
          base = base_from_host_and_path[_URLs.host, _URLs.path]
        when valid_scp_url
          # SCP-style _URLs for private git repos
          _URLs = valid_scp_url[_URLs]
          base = base_from_host_and_path[_URLs.host, _URLs.path]
        when %r{(?:git|ssh|https?|[a-z0-9_-]+@([-\w.]+)):(\/\/)?(.*?)(\.git)?(\/?|\#[-\d\w._]+?)$}i
          # Additional SCP-style _URLs for private git repos
          ...
          raise Informative, "Couldn't determine repo name for _URLs: #{_URLs}"
        end
      ...
      # Returns hostname for for `trunk` _URLs.
      #
