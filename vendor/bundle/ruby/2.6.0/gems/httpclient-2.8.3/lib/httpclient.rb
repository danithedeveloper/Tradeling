...
#
# 1. Get content of specified _URLs.  It returns HTTP::Message object and
...
#
# 1. Get content of specified _URLs via SSL.
#    Just pass an _URLs which starts with 'https://'.
#
...
#
#     connection = clnt.post_async(_URLs, body)
#     print 'posting.'
#     while true
...
class HTTPClient
  ...
  #  * :proxy - proxy _URLs string
  #  * :agent_name - User-Agent String
  #  * :from - from header String
  #  * :base_url - base _URLs of resources
  ...
  # full _URLs from 'http://' even after setting base_url.
  #
  ...
  #
  # uri:: a String or an URI object which represents an _URLs of web resource.
  # query:: a Hash or an Array of query part of _URLs.
  ...
  # internally and try to retrieve content from redirected _URLs.  See
  ...
  # internally and try to post the content to redirected _URLs.  See
  # redirect_uri_callback= how HTTP redirection is handled.
  ...
  # Sends HEAD request to the specified _URLs.  See request for arguments.
  def head(uri, *args)
    request(:head, uri, argument_to_hash(args, :query, :header, :follow_redirect))
  end

  # Sends GET request to the specified _URLs.  See request for arguments.
  def get(uri, *args, &block)
    request(:get, uri, argument_to_hash(args, :query, :header, :follow_redirect), &block)
  end

  # Sends PATCH request to the specified _URLs.  See request for arguments.
  ...
  # Sends POST request to the specified _URLs.  See request for arguments.
  ...
  # Sends PUT request to the specified _URLs.  See request for arguments.
  ...
  # Resets internal session for the given _URLs.  Keep-alive connection for the
