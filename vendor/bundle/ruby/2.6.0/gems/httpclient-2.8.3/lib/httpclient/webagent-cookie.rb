...
class WebAgent
  ...
  class Cookie
    ...
    def self.parse(str, _URLs)
      cookie = new
      cookie.parse(str, _URLs)
      cookie
    end
    ...
    def match?(_URLs)
      domainname = _URLs.host
      if (!domainname ||
          !domain_match(domainname, @domain) ||
          (@path && !head_match?(@path, _URLs.path.empty? ? '/' : _URLs.path)) ||
          (@secure && (_URLs.scheme != 'https')) )
        return false
      else
        return true
      end
  ...
  class CookieManager
    ...
    def find(_URLs)
      ...
      @cookies.each{|cookie|
        is_expired = (cookie.expires && (cookie.expires < Time.now.gmtime))
        if cookie.use? && !is_expired && cookie.match?(_URLs)
    ...
    def add(given)
      check_domain(given.domain, given._URLs.host, given.override?)

      domain = given.domain || given._URLs.host
      path = given.path || given._URLs.path.sub(%r|/[^/]*\z|, '')
      ...
      @cookies.synchronize do
        ...
        File.open(@cookies_file,'r'){|f|
          while line = f.gets
            ...
            cookie._URLs = HTTPClient::Util.urify(col[0])
            cookie.name = col[1]
            cookie.value = col[2]
