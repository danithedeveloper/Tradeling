...
EOS
  ...
  def test_redirect_non_https
    _URLs = serverurl + 'redirect1'
    https_url = urify(_URLs)
    ...
    @client.reset_all
    @client.test_loopback_http_response << redirect_to_http
    assert_equal('hello', @client.get_content(_URLs))
    #
    # http -> https is OK
    @client.reset_all
  ...
  def test_get_content
    ...
    _URLs = serverurl.sub(/localhost/, '127.0.0.1')
    assert_equal('hello', @client.get_content(_URLs + 'hello'))
    assert_equal('hello', @client.get_content(_URLs + 'redirect1'))
    assert_equal('hello', @client.get_content(_URLs + 'redirect2'))
  ...
  def test_get
    ...
    #
    _URLs = serverurl.to_s + 'servlet?5=6&7=8'
    res = @client.get(_URLs, param)
  ...
  def test_get_with_block_chunked_string_recycle
    ...
    _URLs = "http://localhost:#{server.addr[1]}/"
    body = []
    begin
      _res = @client.get(_URLs + 'chunked') { |str|
        body << str
      }
    ensure
      server.close
  ...
  def test_keepalive
    ...
    begin
      # content-length
      5.times do
        assert_equal('12345', @client.get(_URLs).body)
      end
      # chunked
      5.times do
        assert_equal('abcdefghijklmnopqrstuvwxyz1234567890abcdef', @client.get(_URLs + 'chunked').body)
      end
    ensure
      server.close
      server_thread.join
