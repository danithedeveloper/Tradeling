...
class TestSSL < Test::Unit::TestCase
  ...
  def setup
    ...
    setup_server
    setup_client
    @_URLs = "https://localhost:#{serverport}/hello"
  end
  ...
  def test_proxy_ssl
    ...
    escape_noproxy do
      ...
      assert_equal(200, @client.get(@_URLs).status)
...
-----END RSA PRIVATE KEY-----
  ...
  def test_tcp_keepalive
    ...
    session = @client.instance_variable_get(:@session_manager).send(:get_cached_session, HTTPClient::Site.new(URI.parse(@_URLs)))
  ...
  def test_timeout
    _URLs = "https://localhost:#{serverport}/"
    @client.ssl_config.add_trust_ca(path('ca-chain.pem'))
    assert_equal('sleep', @client.get_content(_URLs + 'sleep?sec=2'))
    @client.receive_timeout = 1
    @client.reset_all
    assert_equal('sleep', @client.get_content(_URLs + 'sleep?sec=0'))

    start = Time.now
    assert_raise(HTTPClient::ReceiveTimeoutError) do
      @client.get_content(_URLs + 'sleep?sec=5')
    end
    if Time.now - start > 3
