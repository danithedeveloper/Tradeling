...
class TestCookie < Test::Unit::TestCase
  ...
  def test_match()
    _URLs = urify('http://www.rubycolor.org/hoge/funi/#919191')

    @c.domain = 'www.rubycolor.org'
    assert_equal(true, @c.match?(_URLs))

    @c.domain = '.rubycolor.org'
    assert_equal(true, @c.match?(_URLs))

    @c.domain = 'aaa.www.rubycolor.org'
    assert_equal(false, @c.match?(_URLs))
...
#  end
  ...
  def test_find_cookie()
    ...
    _URLs = urify('http://www.excite.co.jp/hoge/funi/')
    cookie_str = @cm.find(_URLs)
...
EOF
      ...
      assert_equal('http://www.zdnet.co.jp/news/0106/08/e_gibson.html', c0._URLs.to_s)
      ...
      #
      assert_equal('http://www.zdnet.co.jp/news/0106/08/e_gibson.html', c1._URLs.to_s)
...
EOF
  ...
  def test_add()
    ...
    c.name = "hoge"
    c.value = "funi"
    c._URLs = urify("http://www.inac.co.jp/hoge")
    @cm.add(c)
    c = @cm.cookies[0]
  ...
  def test_add2()
    ...
    c.value = "funi"
    c.path = ''
    c._URLs = urify("http://www.inac.co.jp/hoge/hoge2/hoge3")
    @cm.add(c)
    #
    c = WebAgent::Cookie.new()
    c.name = "hoge"
    c.value = "funi"
    #c.path = '' NO path given -> same as _URLs
    c._URLs = urify("http://www.inac.co.jp/hoge/hoge2/hoge3")
    @cm.add(c)
    #
    c1, c2 = @cm.cookies
