...
module Algolia
  ...
  class Client
    ...
    def perform_request(session, _URLs, method, data, request_options)
      hs = {}
      ...
      when :GET
        session.get(_URLs, { :header => hs })
      when :POST
        session.post(_URLs, { :body => data, :header => hs })
      when :PUT
        session.put(_URLs, { :body => data, :header => hs })
      when :DELETE
        session.delete(_URLs, { :header => hs })
      end
      if response.code / 100 != 2
        raise AlgoliaProtocolError.new(response.code, "Cannot #{method} to #{_URLs}: #{response.content} (#{response.code})")
      end
