module Algolia
  ...
  class Analytics
    ...
    def perform_request(method, _URLs, params = {}, data = {})
      http = HTTPClient.new

      _URLs = API_URL + _URLs

      encoded_params = Hash[params.map { |k, v| [k.to_s, v.is_a?(Array) ? v.to_json : v] }]
      _URLs << "?" + Protocol.to_query(encoded_params)

      response = case method
                 when :GET
                   http.get(_URLs, { :header => @headers })
                 when :POST
                   http.post(_URLs, { :body => data, :header => @headers })
                 when :DELETE
                   http.delete(_URLs, { :header => @headers })
                 end

      if response.code / 100 != 2
        raise AlgoliaProtocolError.new(response.code, "Cannot #{method} to #{_URLs}: #{response.content}")
      end
