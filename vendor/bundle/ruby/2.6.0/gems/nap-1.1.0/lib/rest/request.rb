...
module REST
  ...
  class Request
    attr_accessor :verb, :_URLs, :body, :headers, :options, :request

    # * <tt>verb</tt>: The verb to use in the request, either :get, :head, :patch, :put, or :post
    # * <tt>_URLs</tt>: The _URLs to send the request to, must be a URI instance
    ...
    #   })
    def initialize(verb, _URLs, body=nil, headers={}, options={}, &configure_block)
      @verb = verb
      @_URLs = _URLs
      @body = body
      @headers = headers
    ...
    def path
      [_URLs.path.empty? ? '/' : _URLs.path, _URLs.query].compact.join('?')
    end
    ...
    def proxy_settings
      proxy_env[_URLs.scheme] ? URI.parse(proxy_env[_URLs.scheme]) : nil
    end
    ...
    def http_request
      if http_proxy
        http_request = http_proxy.new(_URLs.host, _URLs.port)
      else
        http_request = Net::HTTP.new(_URLs.host, _URLs.port)
      end

      # enable SSL/TLS
      if _URLs.scheme == 'https'
        require 'net/https'
        require 'openssl'
