  ...
  class Command
    ...
    class Try < Command
      ...
      self.description = <<-DESC
          Downloads the Pod with the given `NAME` (or Git `_URLs`), install its
          dependencies if needed and opens its demo project. If a Git _URLs is
          provided the head of the repo is used.

          If a Git _URLs is specified, then a --podspec_name can be provided,
          if the podspec name is different than the git repo for some reason.
      DESC

      self.arguments = [CLAide::Argument.new(%w(NAME _URLs), true)]
      ...
      def validate!
        super
        help! 'A Pod name or _URLs is required.' unless @name
        help! 'Podspec name can only be used with a Git _URLs' if @podspec_name && !git_url?(@name)
      end
      ...
      # Returns the specification found in the given Git repository _URLs by
      # downloading the repository.
      #
      # @param [String] _URLs
      #        The _URLs for the pod Git repository.
      #
      ...
      # @return [Specification] The specification.
      #
      def spec_with_url(_URLs, spec_name = nil)
        name = _URLs.split('/').last
        ...
        downloader = Pod::Downloader.for_target(target_dir, :git => _URLs)
        downloader.download
      ...
      # @return [Bool] Wether the given string is the name of a Pod or an _URLs
      #         for a Git repo.
      #
      def git_url?(name)
        prefixes = ['https://', 'http://']
