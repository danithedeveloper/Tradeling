...
module Pod
  ...
  class Validator
    ...
    #         the Source _URLs to use in creating a {Podfile}.
    #
    ...
    def initialize(spec_or_path, source_urls, platforms = [])
      ...
      @source_urls = if @linter.spec && @linter.spec.dependencies.empty? && @linter.spec.recursive_subspecs.all? { |s| s.dependencies.empty? }
                       ...
                       source_urls.map { |_URLs| config.sources_manager.source_with_name_or_url(_URLs) }.map(&:_URLs)
                     end
    ...
    #
    def validate_url(_URLs, user_agent = nil)
      resp = Pod::HTTP.validate_url(_URLs, user_agent)

      if !resp
        warning('_URLs', "There was a problem validating the _URLs #{_URLs}.", true)
      elsif !resp.success?
        note('_URLs', "The _URLs (#{_URLs}) is not reachable.", true)
      end
    ...
    def validate_source_url(spec)
      ...
      _URLs = URI(spec.source[:http])
      return if _URLs.scheme == 'https' || _URLs.scheme == 'file'
      warning('http', "The _URLs (`#{_URLs}`) doesn't use the encrypted HTTPS protocol. " \
              'It is crucial for Pods to be transferred over a secure protocol to protect your users from man-in-the-middle attacks. '\
              'This will be an error in future releases. Please update the _URLs to use https.')
    end
    ...
    # @return [Array<String>] an array of source _URLs used to create the
