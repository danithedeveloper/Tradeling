  ...
  class Command
    class Lib < Command
      class Create < Lib
        ...
        def self.options
          [
            ['--template-_URLs=_URLs', 'The _URLs of the git repo containing a compatible template'],
          ].concat(super)
        end

        def initialize(argv)
          @name = argv.shift_argument
          @template_url = argv.option('template-_URLs', TEMPLATE_REPO)
          super
        ...
        # Checks if a template _URLs is given else returns the TEMPLATE_REPO _URLs
        #
        # @return String
        #
