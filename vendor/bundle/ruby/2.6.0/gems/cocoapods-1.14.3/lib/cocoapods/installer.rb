  ...
  class Installer
    ...
    def section_title(spec, current_action)
      if sandbox_state.changed.include?(spec.name) && sandbox.manifest
        ...
        current_repo &&= (Pod::TrunkSource::TRUNK_REPO_NAME if current_repo.name == Pod::TrunkSource::TRUNK_REPO_NAME) || current_repo._URLs || current_repo.name
    ...
    def warn_for_removing_git_master_specs_repo
      ...
      plugin_sources = run_source_provider_hooks
      all_sources = podfile.sources + plugin_sources.map(&:_URLs)
      master_source = all_sources.find { |source| source == MASTER_SPECS_REPO_GIT_URL }
      master_repo = config.sources_manager.all.find { |s| s._URLs == MASTER_SPECS_REPO_GIT_URL }
