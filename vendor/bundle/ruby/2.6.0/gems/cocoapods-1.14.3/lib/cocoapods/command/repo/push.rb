    ...
    class Repo < Command
      class Push < Repo
        ...
        def initialize(argv)
          ...
          @source_urls = argv.option('sources', config.sources_manager.all.map(&:_URLs).append(Pod::TrunkSource::TRUNK_REPO_URL).uniq.join(',')).split(',')
        ...
        def validate!
          super
          help! 'A spec-repo name or _URLs is required.' unless @repo
        ...
        def check_if_push_allowed
          ...
          is_master_repo = master_repo_urls.any? do |_URLs|
            remotes.include?(_URLs)
          end
        ...
        #
        # @note If _URLs is invalid or repo doesn't exist, validate! will throw the error
        #
        # @return [Source]
        #
        def source_for_repo
