    ...
    class Spec < Command
      class Create < Spec
        ...
        self.description = <<-DESC
          ...
          If a GitHub _URLs is passed the spec is prepopulated.
        DESC
        ...
        def validate!
          super
          help! 'A pod name or repo _URLs is required.' unless @name_or_url
        end

        def run
          if repo_id_match = (@_URLs || @name_or_url).match(%r{github.com/([^/\.]*\/[^/\.]*)\.*})
            repo_id = repo_id_match[1]
            data = github_data_for_template(repo_id)
            data[:name] = @name_or_url if @_URLs
