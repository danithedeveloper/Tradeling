...
* Change _URLs validation failure to a note  
  [Paul Beusterien](https://github.com/paulb777)
...
* Fix heuristic for determining whether the source _URLs to be added is CDN  
  [Igor Makarov](https://github.com/igor-makarov)
...
* Add CDN repo Source to allow retrieving specs from a web _URLs.  
  [igor-makarov](https://github.com/igor-makarov)
...
* Trying to add a spec repo with a `file://` _URLs on Ruby 2.6 won't fail with a
...
* Accept `pod repo push` with _URLs instead of only repo name.  
  [Mark Schall](https://github.com/maschall)
...
* Fixes an issue showing the _URLs to remote resources in `pod repo list`.  
  [Kyle Fuller](https://github.com/kylef)
...
* Fixes a crash when using file _URLs as a source.  
...
* More robust generation of source names from _URLs.  
  [Samuel Giddins](https://github.com/segiddins)
...
* Doesn't take into account the trailing `.git` in repository _URLs when
...
* Add support for loading podspecs from *only* specific spec-repos via
  ...
  **no** repos will be included by default. For example:
  ...
  Any source _URLs specified that have not yet been added will be cloned before
  resolution begins.  
...
* Fix crash when the _URLs of a private GitHub repo is passed to `pod spec
...
* Numerous improvements to the validator and to the linter.
  * Validate the reachability of screenshot _URLs in podspecs while linting a
    specification.  
    [Kyle Fuller](https://github.com/kylef)
...
* Adds functionality to specify a template _URLs for the `pod lib create` command.  
  [Piet Brauer](https://github.com/pietbrauer)
...
- The final project isn’t affected anymore by the `inhibit_all_warnings!`
  option.
- Support for redirects while using podspec from an _URLs.
...
- Use double quotes when shelling out. This makes a _URLs like `$HOME/local/lib` work. [#396](http://git.io/DnBzhA)
...
Alternatively, you can specify a _URLs where an HTML version of the documentation
can be found:
...
* [#112](https://github.com/CocoaPods/CocoaPods/issues/112): Introduced `--push`
  ...
  push _URLs. The change is preserved in future calls to `$ pod setup`.
...
A dependency can take a git _URLs if the repo contains a podspec file in its
...
# From a podspec that's outside a spec repo _and_ the library’s repo. This can be a file or http _URLs.
